...
class RSA:
    ...
    def encrypt(
        ...
        try:
            ...
            Io.write_bytes(path_to_encrypted, encrypted_key)
            logger.info(f"Encrypted symmetric key saved to {path_to_encrypted}")
    ...
    def decrypt(
        ...
        """
        try:
            symmetric_encrypted = Io.deserialize_symmetric_key(path_to_encrypted)
            ...
            Io.serialize_symmetric_key(decrypted_key, path_to_decrypted)
            logger.info(f"Decrypted symmetric key saved to {path_to_decrypted}")
            return decrypted_key
        except Exception as e:
            logger.error(f"Error during RSA decryption: {e}")
